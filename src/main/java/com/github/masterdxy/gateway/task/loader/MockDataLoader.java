 ...
 * @author tomoyo
 */
@Component public class MockDataLoader extends AbstractScheduledService implements TaskRegistry.Task {
    ...
    private void fetchMockData() {
        if (masterWriteLocker.isHasMasterLock()) {
            logger.info("[MockData] Fetch MockData Config from [MySQL], with MasterWriteLock=[{}]",
            ...
            logger.info("[MockData] Fetch MockData Config from [HAZELCAST], with MasterWriteLock=[{}]",
                masterWriteLocker.isHasMasterLock());
