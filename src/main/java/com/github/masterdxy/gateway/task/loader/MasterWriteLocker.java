 ...
 */
@Component
public class MasterWriteLocker extends AbstractScheduledService implements TaskRegistry.Task {
	...
	private void lock (ILock iLock) {
		try {
			if (iLock.isLocked()) {
				logger.info(lockStatusInfo,
				            iLock.isLocked(),
				            iLock.isLockedByCurrentThread(),
				...
				if (iLock.tryLock(Constant.LOCK_TIMEOUT_MS,
				                  TimeUnit.MILLISECONDS,
				                  Constant.LOCK_LEASE_TIME_MS,
				                  TimeUnit.MILLISECONDS
				                 )) {
					logger.info(lockTrySuccessInfo,
					            iLock.isLocked(),
					            iLock.isLockedByCurrentThread(),
					            iLock.getRemainingLeaseTime()
					           );
					...
					logger.info(lockTryTimeoutInfo,
					            iLock.isLocked(),
					            iLock.isLockedByCurrentThread(),
					            iLock.getRemainingLeaseTime()
					           );
	...
	protected Scheduler scheduler () {
		return Scheduler.newFixedDelaySchedule(Constant.DELAY_TYR_LOCK, Constant.DELAY_TYR_LOCK,
		                                       TimeUnit.MILLISECONDS);
	}
